    ...
    try {
        ...
        if (response.ok) {
            ...
            if (profile.recommendations_data) {
                renderRecommendations(JSON.parse(profile.recommendations_data));
    ...
    function renderRecommendations(data) {
        ...
        if (data.potential_recruiters) {
            aiRecruiters.innerHTML = data.potential_recruiters.map(recruiter => {
                ...
                return `<li style="margin-bottom: 0.8rem; background: rgba(13, 17, 23, 0.6); padding: 1rem; border-radius: 8px; border: 1px solid var(--border-color);">
                            <a href="${searchUrl}" target="_blank" style="color: var(--text-primary); text-decoration: none; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <span style="font-size: 0.9em; background: rgba(88, 166, 255, 0.1); color: #79c0ff; padding: 0.4rem 0.8rem; border-radius: 4px;">View Jobs 🔗</span>
                            </a>
                        </li>`;
            }).join('');
        }
