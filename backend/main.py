...
try:
    ...
    if "location" not in columns:
        cursor.execute("ALTER TABLE profiles ADD COLUMN location VARCHAR DEFAULT ''")
    if "position" not in columns:
        cursor.execute("ALTER TABLE profiles ADD COLUMN position VARCHAR DEFAULT ''")
    ...
    if profile:
        ...
        if last_login != today:
            if last_login == today - timedelta(days=1):
                current_streak = getattr(profile, 'current_streak') or 0
                setattr(profile, 'current_streak', current_streak + 1)
            else:
                setattr(profile, 'current_streak', 1)
            current_streak = getattr(profile, 'current_streak')
            longest_streak = getattr(profile, 'longest_streak') or 0
            if current_streak > longest_streak:
                setattr(profile, 'longest_streak', current_streak)
            setattr(profile, 'last_login_date', today)
            db.commit()
...
def request_password_reset(request: schemas.PasswordResetRequest, db: Session = Depends(database.get_db)):
    reset_token = auth.create_password_reset_token(request.email, db)
    if not reset_token:
...
def reset_password(reset_data: schemas.PasswordReset, db: Session = Depends(database.get_db)):
    success = auth.reset_password(reset_data.token, reset_data.new_password, db)
    if not success:
...
def update_profile(profile_update: schemas.ProfileUpdate, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    setattr(profile, 'skills', profile_update.skills)
    setattr(profile, 'experience', profile_update.experience)
    setattr(profile, 'education', profile_update.education)
    setattr(profile, 'location', profile_update.location)
    setattr(profile, 'position', profile_update.position)
...
async def get_recommendations(current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if not profile or (not skills and not experience):
        raise HTTPException(status_code=400, detail="Profile is incomplete. Please add skills or experience first.")
    
    import json
    recommendations_data = getattr(profile, 'recommendations_data')
    if recommendations_data:
        return json.loads(recommendations_data)
        
    recommendations = get_career_recommendations(
        skills, experience, getattr(profile, 'education', ''), getattr(profile, 'position', '')
    )
    
    setattr(profile, 'recommendations_data', json.dumps(recommendations))
    db.commit()
...
@app.post("/api/chat")
async def chat_endpoint(request: schemas.ChatRequest, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    profile = db.query(models.Profile).filter(models.Profile.user_id == current_user.id).first()
    profile_data = None
    if profile:
        profile_data = {
    ...
    return {"reply": chat_with_coach(request.messages, profile_data)}
...
def get_user_scores(game_name: Optional[str] = None, current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
